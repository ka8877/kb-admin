 ...
 * 원본 인덱스 찾기 함수
 */
export const getOriginalIndex = (searchField: SearchField, searchFields: SearchField[]): number => {
...
export const extractBaseField = (dateField: string): string => {
  // 1. 접두사 패턴 제거: start_, end_, imp_start_, imp_end_ 등 (camelCase 포함)
  let baseField = dateField.replace(
    /^(start_|end_|imp_start_|imp_end_|impStart|impEnd|start|end)/,
    '',
  );
  // 2. 접미사 패턴 제거: _start_date, _end_date, _start, _end, _date (camelCase 포함)
  baseField = baseField.replace(
    /(_start_date$|_end_date$|_start$|_end$|_date$|StartDate$|EndDate$|Start$|End$|Date$|From$|To$)/,
    '',
  );
...
export const groupDateRangeFields = (
  ...
  string,
  {
    start?: Extract<SearchField, { type: 'dateRange' }>;
    end?: Extract<SearchField, { type: 'dateRange' }>;
  }
> => {
  const groups: Record<
    string,
