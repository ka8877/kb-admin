...
import { getOriginalIndex, groupDateRangeFields, getDateFieldName, extractBaseField } from '../utils/searchFieldUtils';
...
const AdvancedFilters = memo<AdvancedFiltersProps>(({
  ...
  const advancedFilterFields = useMemo(
    () => searchFields.filter((sf) => sf.type === 'select' || sf.type === 'radio' || sf.type === 'dateRange'),
    [searchFields],
  );
