...
const transformItem = (
  v: Partial<UserRoleChangeItem> & Record<string, any>,
  ...
  return {
    ...
    changeType: v.changeType ?? v.change_type ?? '',
    itsvcNo: v.itsvcNo ?? v.itsvc_no ?? '',
    reason: v.reason ?? '',
    beforeState: v.beforeState ?? v.before_state ?? '{}',
    afterState: v.afterState ?? v.after_state ?? '{}',
    changedBy: v.changedBy ?? v.changed_by ?? '',
    changedAt: v.changedAt ?? v.changed_at ?? '',
  };
};
...
const transformUserRoleChanges = (raw: unknown): UserRoleChangeItem[] => {
  ...
  if (Array.isArray(raw)) {
    return raw
      .map((item, index) => {
        if (!item) return null;
        const v = item as Partial<UserRoleChangeItem> & Record<string, any>;
  ...
  if (typeof raw === 'object' && raw !== null) {
    ...
    return entries.map(([key, value], index) => {
      const v = value as Partial<UserRoleChangeItem> & Record<string, any>;
