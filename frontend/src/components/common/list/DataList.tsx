...
import type { GridColDef, GridPaginationModel, GridRowSelectionModel, GridValidRowModel } from '@mui/x-data-grid';
...
const defaultGetRowId = <T extends GridValidRowModel,>(getter: DataListProps<T>['rowIdGetter']) => (row: T) => {
...
const DataList = <T extends GridValidRowModel = GridValidRowModel>({
  ...
  useEffect(() => {
    ...
    if (fetcher) {
      let mounted = true;
      fetcher().then((d) => mounted && setData(d)).catch(() => {});
      return () => {
        mounted = false;
      };
    }
  ...
  const filteredRows = useMemo(() => {
    ...
    return data.filter((row) => {
      if (!searchField) {
        return Object.values(row).some((v) => (v == null ? false : String(v).toLowerCase().includes(q)));
      }
  ...
  const handleExportAll = () => {
    ...
    const csv = [headers.join(','), ...filteredRows.map((r) => headers.map((h) => escapeCell((r as any)[h])).join(','))].join('\r\n');
  ...
  return (
    <Box>
      <ListSearch
        columns={columns}
        onSearch={(p) => {
          setSearchField(p.field);
          setSearchQuery(p.query);
          setPaginationModel((p0) => ({ ...p0, page: 0 }));
        }}
      ...
      <ListActions
        ...
        selectedIds={Array.isArray(selectionModel) ? selectionModel : selectionModel ? [selectionModel] : []}
        onCreate={onCreate}
